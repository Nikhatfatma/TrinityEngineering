    ...
    <div className="group w-full rounded-xl border border-gray-100 bg-white p-6 shadow-sm transition-all duration-500 hover:border-blue-100 hover:shadow-xl max-lg:rounded-2xl max-lg:p-5 md:p-8">
      ...
      <div className={`space-y-4 ${HOME_POINT_BODY_CLASS}`}>
        {paragraphs.map((paragraph, index) => (
          <p key={index} className="opacity-80 transition-opacity group-hover:opacity-100">
            {paragraph}
          </p>
        ))}
      </div>
...
export default function DroneApproachSection() {
  return (
    <section
      id={DRONE_APPROACH.id}
          ...
          <div className="grid w-full min-w-0 grid-cols-1 items-start gap-6 max-lg:gap-y-4 md:gap-8 lg:grid-cols-[minmax(0,1fr)_44%] xl:grid-cols-[minmax(0,1fr)_46%] xl:gap-12">
            ...
            <div className="max-lg:col-span-full min-w-0 w-full">
              <div className="space-y-4 md:space-y-6">
                {DRONE_APPROACH.approachBlocks.map((block) => (
                  <TextBlock
                    key={block.eyebrow}
            ...
            >
              <img
                src={DRONE_APPROACH.trainingImageSrc}
                alt={DRONE_APPROACH.trainingImageAlt}
